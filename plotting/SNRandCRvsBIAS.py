...
for value in compression_values:
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        cwd=os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'level_3')
    )
...
plt.savefig(os.path.join(output_dir, 'compression_rates_vs_bias.pdf'), format='pdf', bbox_inches='tight')
...
plt.grid(True, alpha=0.3)
plt.savefig(os.path.join(output_dir, 'snr_vs_bias.pdf'), format='pdf', bbox_inches='tight')